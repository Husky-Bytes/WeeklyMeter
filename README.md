...
**[Android APK 다운로드 · 0.6.0](https://github.com/Husky-Bytes/WeeklyMeter/releases/download/v0.6.0/WeeklyMeter-0.6.0.apk)** · [변경 내용 / 릴리스](https://github.com/Husky-Bytes/WeeklyMeter/releases/tag/v0.6.0) · [문제 제보 / 제안](https://github.com/Husky-Bytes/WeeklyMeter/issues)
...
자동 조회는 15 / 30 / 60분 중 선택하며 홈 위젯이 있거나 플로팅 위젯이 실행 중일 때 예약합니다. 성공한 자동 조회는 사용량과 마지막 성공 조회 시각을 두 표시 방식에 반영합니다. 플로팅 위젯은 별도의 조회 주기를 만들지 않습니다. **앱을 열거나 언어·꾸미기·크기 설정만 바꾸거나 플로팅 창만 띄워서는 네트워크 조회를 시작하지 않습니다.** Android 절전·네트워크 제한 때문에 자동 조회가 늦어질 수 있으며 실시간 표시를 보장하지 않습니다.
...
0.6.0은 선택적 플로팅 위젯, 홈 위젯 3회 탭, 독립적인 꾸미기·크기 설정을 추가합니다. 기존 홈 위젯·로그인·절전 진단은 유지하며 플로팅 표시용 권한과 실행 중 서비스가 추가됩니다. [전체 변경 안내 · 한국어 / English](CHANGELOG-0.6.0.md)

Windows Android 전체 빌드·APK 정렬·서명 확인, **12,601개 실행 검사**, 아이콘 정적 검사 49개와 XML 18개·리소스 검사를 통과했습니다. [상세 검사 기록](TEST-RESULTS.txt). 로컬 로직·대체 Android 환경·소스 계약 검사는 실기기 플로팅 표시·실계정 성공이나 절전 중 자동 조회 문제의 해결을 보장하지 않습니다.

별도로 **Android 15(API 35) 에뮬레이터 실행 검사 31개**를 통과했습니다. 배포 APK 설치, 설정 화면·실제 비트맵 미리보기, 플로팅 창 부착, 가상 사용량 변경 반영, 크기 변경, 이동·길게 눌러 닫기·다시 열기, 로그아웃 상태의 탭 조회 실패 경로를 확인했습니다. 네트워크를 끄고 테스트용 표시 권한과 가상 데이터를 사용했습니다. 스크린샷 시각 검수, 미리보기의 실제 스크롤 고정, 권한 허용 화면, 물리 잠금 해제, 홈 런처의 3회 탭 전체 동작은 검증하지 않았습니다.

<details>
<summary>직접 빌드 / APK 무결성 확인</summary>

Windows 빌드 환경: JDK 21.0.8, Android platform 36, build-tools 35.0.0, Python 3. 앱은 minSdk 26 / targetSdk 35, 패키지 `dev.yerin.weeklymeter`, versionCode 10입니다.
...
배포 APK `WeeklyMeter-0.6.0.apk`의 SHA-256:
