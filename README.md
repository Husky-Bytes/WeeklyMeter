...
**Codex 주간 잔여량, 홈 화면과 플로팅 위젯에서 한눈에.**

PC나 외부 중계 서버 없이 폰에서 동작하는 Android 위젯입니다. `n%`만 작게 두거나, 초기화 시각과 마지막 조회 시각을 함께 표시할 수 있습니다. 홈 화면 위젯과 다른 앱 위에 띄우는 플로팅 위젯을 따로 꾸밀 수 있습니다.

[한국어](README.md) · [English](README.en.md)

**[Android APK 다운로드 · 0.6.1](https://github.com/Husky-Bytes/WeeklyMeter/releases/download/v0.6.1/WeeklyMeter-0.6.1.apk)** · [변경 내용 / 릴리스](https://github.com/Husky-Bytes/WeeklyMeter/releases/tag/v0.6.1) · [문제 제보 / 제안](https://github.com/Husky-Bytes/WeeklyMeter/issues)
...
- **다른 앱 위에 플로팅 위젯** — 홈 위젯을 빠르게 3번 눌러 띄우고, 끌어서 이동합니다. 홈 위젯과 별도로 꾸미고 너비·높이도 조절합니다. 사용자가 표시 권한을 허용해야 합니다.
...
- **전체 투명도까지 따로 조절** — 배경뿐 아니라 글자·로고·조회 효과를 한꺼번에 흐리게 할 수 있습니다. 홈·플로팅 설정은 독립적이며 기존 배경 불투명도도 유지합니다.
- **내 홈 화면에 맞는 스타일** — 컬러휠, 글꼴, 행별 글자 크기, 순서, 위치, 내부 여백을 조절합니다.
- **날짜도 필요한 부분만** — 초기화와 마지막 성공 조회 시각을 각각 설정합니다. 날짜 없이 시간만 표시해도 됩니다.
- **설정 중에도 고정 미리보기** — 설명이 보이는 4개 설정 카드와 별도 요소 선택으로 편집합니다. 홈 스타일은 모든 홈 위젯에 공통 적용되며 플로팅 스타일과는 독립적입니다.
...
앱의 **자동 조회 간격**에서 **15~10080분(7일)** 사이의 정수를 직접 입력합니다. 15 / 30 / 60분 단축 버튼도 있습니다. 범위 밖 값이나 소수는 저장하지 않고 입력창에서 수정하도록 안내합니다. 홈 위젯과 플로팅 위젯은 같은 간격을 공유하며, 홈 위젯이 있거나 플로팅 세션이 실행 중일 때 예약합니다.

성공한 자동 조회는 사용량과 마지막 성공 조회 시각을 두 표시 방식에 반영합니다. 플로팅 위젯은 별도의 조회 주기를 만들지 않습니다. **앱을 열거나 간격·언어·꾸미기·크기를 저장하거나 플로팅 창만 띄워서는 사용량을 조회하지 않습니다.** 간격 변경은 다음 예약에 반영합니다. Android 주기 작업의 최소 간격이 15분이므로 그보다 짧게 설정할 수 없으며, 절전·네트워크 제한으로 실제 실행은 늦어질 수 있습니다. 정확한 시각·실시간 표시를 보장하지 않습니다.

## 플로팅 위젯 사용하기

1. 앱의 **플로팅 위젯 → 띄우기**를 누릅니다. 처음에는 안내에 따라 시스템 설정에서 **다른 앱 위에 표시**를 직접 허용합니다. 홈 위젯만 사용할 때는 이 권한이 필요하지 않습니다.
2. 이후에는 **같은 홈 위젯을 0.9초 안에 3번** 눌러 띄울 수도 있습니다. 첫 탭은 기존처럼 새로고침을 요청하고, 두 번째·세 번째 탭은 별도 조회를 추가하지 않습니다.
3. 플로팅 위젯을 **한 번 누르면 새로고침**, **끌면 이동**, **0.6초간 길게 누르면 닫기**입니다. 앱의 플로팅 메뉴나 서비스 알림에서도 닫을 수 있습니다.
4. **플로팅 위젯 → 꾸미기·크기**에서 글꼴·색·행·날짜·조회 효과를 홈 위젯과 독립적으로 설정합니다. 고정 미리보기의 **크기 조절**을 누르면 너비·높이 설정으로 이동합니다.

기본 크기는 **128×96dp**, 너비는 **48~360dp**, 높이는 **48~300dp**입니다. 화면에 들어가지 않는 크기는 표시 가능한 영역에 맞춥니다. 미리보기는 선택한 가로세로 비율을 유지합니다. 플로팅 기본값 복원은 홈 위젯 설정을 바꾸지 않습니다.

**배경·투명도 → 전체 불투명도**는 배경·글자·로고·조회 효과에 함께 적용됩니다. **100%는 기존 모습, 0%는 완전 투명**입니다. 배경만 투명하게 하려면 별도의 **배경 불투명도**를 조절합니다. 전체 0%에서도 플로팅 창과 터치 영역은 남으므로 앱의 **플로팅 위젯 → 닫기**로 종료할 수 있습니다.
...
**실계정 로그인·토큰 갱신, 자격증명의 실제 Android Keystore 저장, 물리 기기의 APK 설치, S25 Ultra / One UI의 렌더링·절전·재부팅·장시간 동작은 미검증입니다.** Android 15 에뮬레이터에서는 APK 설치와 아래 플로팅 기본 동작을 확인했습니다. 자세한 범위는 [보안 범위](SECURITY.md)와 [실제 검사 기록](TEST-RESULTS.txt)을 확인하세요.
...
- 색상은 컬러휠·밝기·HEX·최근 색으로 고릅니다. 전체 불투명도와 배경 불투명도는 각각 0~100%로 설정합니다. 기존 스타일의 전체 불투명도는 100%로 시작하므로 업데이트만으로 모습이 바뀌지 않습니다. 모서리·크기 자동 맞춤도 설정합니다.
- 고정 미리보기 아래에서 글자·날짜 / 배치·여백 / 배경·투명도 / 조회 효과의 4개 설명 카드를 선택합니다. 꾸밀 요소는 별도 선택하며 다른 요소의 긴 목록은 펼치지 않습니다. 중복 설명은 줄이고 간결한 명사형·존댓말 문구를 사용합니다.
...
- 홈 위젯은 1×1 / 2×1 예시, 플로팅은 설정한 너비·높이 비율로 미리 봅니다. 넘침 경고가 있으며 실제 화면 크기·배경·글자 배율은 미리보기와 다를 수 있습니다.
...
완료 표시의 기본 지속시간은 **1.0초**입니다. 홈·플로팅별로 0.1~10.0초를 0.1초 단위 슬라이더·직접 입력으로 설정하거나 반응 표시를 끌 수 있습니다. Android 프로세스 종료·절전·런처 지연으로 정확한 설정 시간에 사라지는 것은 보장하지 않으며 다음 렌더 때 만료를 다시 검사합니다.
...
0.6.1은 글자·로고까지 함께 적용하는 전체 불투명도와 자동 조회 간격 직접 입력을 추가합니다. 기존 플로팅·홈 위젯·로그인·절전 진단과 권한은 유지합니다. [전체 변경 안내 · 한국어 / English](CHANGELOG-0.6.1.md)

Windows Android 전체 빌드·APK 정렬·서명 확인, **13,106개 실행 검사**, 아이콘 정적 검사 49개와 XML 18개·리소스 검사를 통과했습니다. [상세 검사 기록](TEST-RESULTS.txt). 로컬 로직·대체 Android 환경·소스 계약 검사는 실기기 플로팅 표시·실계정 성공이나 절전 중 자동 조회 문제의 해결을 보장하지 않습니다.

별도로 **0.6.1 APK의 Android 15(API 35) 에뮬레이터 실행 검사 62개**를 통과했습니다. 실제 비트맵에서 배경·글자·성공 표시의 100/50/0% 불투명도, 열린 플로팅의 즉시 변경과 홈 설정 분리, 실제 간격 입력창의 47분 저장·14분 거부·취소·단축 버튼을 확인했습니다. 기존 플로팅 부착·이동·닫기 검사도 다시 통과했습니다. 네트워크 없이 가상 데이터를 사용했으며 실제 47분 주기 실행, 로고 픽셀·불투명도 슬라이더 직접 조작, 스크린샷 시각 검수, 권한 화면·실제 잠금·홈 런처 3회 탭은 미검증입니다.

<details>
<summary>직접 빌드 / APK 무결성 확인</summary>

Windows 빌드 환경: JDK 21.0.8, Android platform 36, build-tools 35.0.0, Python 3. 앱은 minSdk 26 / targetSdk 35, 패키지 `dev.yerin.weeklymeter`, versionCode 11입니다.
...
배포 APK `WeeklyMeter-0.6.1.apk`의 SHA-256:
