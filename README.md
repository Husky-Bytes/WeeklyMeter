...
**[Android APK 다운로드 · 0.4.0](https://github.com/Husky-Bytes/WeeklyMeter/releases/download/v0.4.0/WeeklyMeter-0.4.0.apk)** · [변경 내용 / 릴리스](https://github.com/Husky-Bytes/WeeklyMeter/releases/tag/v0.4.0) · [문제 제보 / 제안](https://github.com/Husky-Bytes/WeeklyMeter/issues)
...
- **설정 중에도 고정 미리보기** — 필요한 설정 항목만 열어 편집합니다. 스타일은 모든 위젯에 공통 적용됩니다.
...
4. 홈 화면에 WeeklyMeter 위젯을 추가합니다. 앱의 **위젯 꾸미기**에서 원하는 표시를 고르면 자동 저장됩니다.
5. 위젯을 눌러 새로 조회합니다. **마지막 성공 조회 시각**을 켜 두면 퍼센트가 그대로여도 갱신 여부를 알 수 있습니다.

자동 조회는 15 / 30 / 60분 중 선택하며 위젯이 있을 때만 예약합니다. 성공한 자동 조회도 사용량과 조회 시각에 반영됩니다. **앱을 열거나 설정만 바꿔서는 네트워크 조회를 시작하지 않습니다.** Android 절전·네트워크 제한 때문에 자동 조회가 늦어질 수 있으며 실시간 표시를 보장하지 않습니다.
...
- 고정 미리보기 아래에서 필요한 설정과 요소만 골라 편집합니다. 다른 요소의 긴 설정 목록을 함께 펼치지 않습니다.
...
0.4.0은 위젯 수동 조회를 짧은 직접 실행 서비스로 분리하고, 내부 여백 조절과 항목별 꾸미기 화면을 추가했습니다. 이전 글꼴 경로 수정과 로그인·사용량·꾸미기 설정은 유지합니다. [전체 변경 안내](CHANGELOG-0.4.0.md)

Windows 전체 Android 빌드, APK 생성·정렬·서명 확인, **1,081개 실행 검사**와 10개 XML 파일·리소스 일관성 검사를 통과했습니다. 이 수치는 로컬 로직·파일 검사와 대체 Android 환경 테스트이며, 실기기·실계정 성공을 뜻하지 않습니다. [검사 항목과 미검증 범위](TEST-RESULTS.txt)

<details>
<summary>직접 빌드 / APK 무결성 확인</summary>

Windows 검증 환경: JDK 21.0.8, Android platform 36, build-tools 35.0.0, Python 3. 앱은 minSdk 26 / targetSdk 35, 패키지 `dev.yerin.weeklymeter`, versionCode 6입니다.
...
배포 APK `WeeklyMeter-0.4.0.apk`의 SHA-256:
