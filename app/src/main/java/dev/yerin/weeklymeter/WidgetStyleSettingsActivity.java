...
public final class WidgetStyleSettingsActivity extends Activity {
    ...
    private final List<ControlUpdate> controlUpdates=new ArrayList<>();
    ...
    private LinearLayout layoutElements;
    ...
    private boolean updatingControls,compactPreview,tinyPreview,floating,appearanceDirty;
    private boolean framePosted,controlsDirty,previewDirty;
    private int floatingWidth,floatingHeight;
    private int savedFloatingWidth,savedFloatingHeight;
    private String savedAppearance;
    ...
    private final Runnable endEffect=()->{feedbackState="none";requestUpdates(false,true);};
    private final Runnable renderFrame=()->{
        framePosted=false;
        boolean controls=controlsDirty,preview=previewDirty;controlsDirty=false;previewDirty=false;
        if(controls)refreshControls();
        if(preview)updatePreview();
    };
    private static final class ControlUpdate {
        final View view;final Runnable reflect;
        ControlUpdate(View view,Runnable reflect){this.view=view;this.reflect=reflect;}
    }
    ...
    private void initializeNames(){
        ...
        CATEGORY_NAMES=new String[]{tr("글자·날짜","Text & dates"),tr("배치·여백","Layout & spacing"),tr("배경·투명도","Background & opacity"),tr("조회 효과","Refresh feedback")};
        CATEGORY_HINTS=new String[]{tr("글꼴, 크기, 색, 날짜 요소","Fonts, size, color, date parts"),tr("위치, 순서, 정렬, 안쪽 여백","Position, order, alignment, padding"),tr("전체·배경 불투명도, 색, 모서리","Whole/background opacity, color, corners"),tr("조회 상태와 표시 시간","Status indicators and duration")};
    ...
    @Override public void onCreate(Bundle state){
        ...
        savedFloatingWidth=floatingWidth;savedFloatingHeight=floatingHeight;savedAppearance=WidgetAppearance.signature(current);
        ...
        setContentView(root);root.requestFocus();buildSettings();requestUpdates(true,true);
    }
    @Override protected void onStart(){super.onStart();if(!Texts.locale(this).getLanguage().equals(displayedLanguage)){recreate();return;}if(!floating)readHomePreviewSizes();requestUpdates(true,true);}
    ...
    @Override protected void onStop(){ui.removeCallbacks(endEffect);feedbackState="none";flush();cancelUpdates();super.onStop();}
    @Override protected void onDestroy(){ui.removeCallbacks(publish);ui.removeCallbacks(endEffect);cancelUpdates();super.onDestroy();}
    ...
    private void showSelectedPage(){
        ensurePage(selectedCategory);
        if(selectedCategory==0&&textPanels[selectedElement]==null)buildTextElement(selectedElement);
        if(selectedCategory==1&&layoutPanels[selectedElement]==null)buildLayoutElement(selectedElement);
    ...
    private void paintSelected(Button button,boolean selected){if(button.isSelected()==selected)return;button.setSelected(selected);button.setTextColor(selected?BG:TEXT);button.setBackground(round(selected?ACCENT:0xff28313d,10));}
    private void buildSettings(){
        controlUpdates.clear();settings.removeAllViews();
        Arrays.fill(pages,null);Arrays.fill(textPanels,null);Arrays.fill(layoutPanels,null);layoutElements=null;
        attachNavigation();
        showSelectedPage();
    }
    private void ensurePage(int category){
        if(pages[category]!=null)return;
        pages[category]=column(settings);
        if(category==0)return;
        if(category==1){buildLayoutPage();return;}
        if(category==2){buildBackgroundPage();return;}
        buildFeedbackPage();
    }
    private void buildLayoutPage(){
        ...
        LinearLayout manual=column(spacing);reflect(spacing,()->manual.setVisibility(current.automaticPadding?View.GONE:View.VISIBLE));numeric(manual,tr("좌우 여백","Horizontal padding"),0,32,.5f,()->current.paddingHorizontalDp,value->current.paddingHorizontalDp=value,"dp",true);numeric(manual,tr("상하 여백","Vertical padding"),0,32,.5f,()->current.paddingVerticalDp,value->current.paddingVerticalDp=value,"dp",true);numeric(manual,tr("요소 사이 간격","Row spacing"),0,16,.5f,()->current.rowGapDp,value->current.rowGapDp=value,"dp",true);
        text(spacing,tr("자동 여백을 끄면 직접 조절할 수 있습니다.","Turn off automatic padding to adjust padding and spacing yourself."),11,MUTED,false);
    }
    private void buildBackgroundPage(){
        LinearLayout whole=section(pages[2],tr("위젯 전체","Whole widget"));numeric(whole,tr("전체 불투명도","Overall opacity"),0,100,1,()->current.overallOpacity,value->current.overallOpacity=Math.round(value),"%",false);text(whole,tr("글자·로고·조회 효과 포함 · 0% 숨김 / 100% 원래 표시","Includes text, logo and feedback · 0% hidden / 100% original appearance"),11,MUTED,false);
        LinearLayout background=section(pages[2],tr("배경","Background"));colorControl(background,tr("배경색","Background color"),()->current.background,value->current.background=value);numeric(background,tr("배경 불투명도","Background opacity"),0,100,1,()->current.opacity,value->current.opacity=Math.round(value),"%",false);numeric(background,tr("모서리 둥글기","Corner radius"),0,32,1,()->current.radius,value->current.radius=Math.round(value),"dp",false);text(background,floating?tr("격자: 투명 영역","Checkerboard: transparent areas"):tr("격자: 투명 영역 · 실제 크기는 홈 화면에 따라 다릅니다.","Checkerboard: transparent areas · actual size varies by home screen."),11,MUTED,false);
    }
    private void buildFeedbackPage(){
    ...
    private void buildLayoutElement(int id){
        WidgetStyle.Row row=current.rows[id];layoutPanels[id]=column(layoutElements);LinearLayout card=section(layoutPanels[id],ROW_NAMES[id]);toggle(card,tr("위젯에 표시","Show on widget"),()->row.enabled,value->row.enabled=value);
        ...
        reflect(orderRow,()->{int position=indexOf(id);setText(orderText,tr("표시 순서 ","Row order ")+(position+1)+" / 4");up.setEnabled(position>0);down.setEnabled(position<3);});
        TextView orderSummary=text(card,"",11,MUTED,false);reflect(orderSummary,()->{StringBuilder sequence=new StringBuilder();for(int element:current.order){if(sequence.length()>0)sequence.append(" → ");sequence.append(ELEMENT_NAMES[element]);if(!current.rows[element].enabled)sequence.append(tr("(숨김)"," (hidden)"));}setText(orderSummary,sequence.toString());});
    }
    ...
    private void numeric(LinearLayout parent,String title,float min,float max,float step,NumberGet get,NumberSet set,String unit,boolean decimal){
        ...
        Runnable reflect=()->{updating[0]=true;float value=get.get();int progress=Math.round((value-min)/step);if(slider.getProgress()!=progress)slider.setProgress(progress);if(!input.hasFocus())setText(input,number(value,decimal));updating[0]=false;};reflect(row,reflect);
        Runnable commit=()->{if(updating[0])return;try{float value=Float.parseFloat(input.getText().toString());if(!Float.isFinite(value)||value<min||value>max)throw new NumberFormatException();value=min+Math.round((value-min)/step)*step;boolean different=Float.compare(get.get(),value)!=0;set.set(value);input.setError(null);updating[0]=true;setText(input,number(value,decimal));updating[0]=false;if(different)changed();else requestUpdates(true,false);flush();}catch(NumberFormatException e){input.setError(number(min,decimal)+"–"+number(max,decimal)+unit+tr(" 범위로 입력해 주세요."," — enter a value in this range."));}};
        input.addTextChangedListener(new TextWatcher(){public void beforeTextChanged(CharSequence s,int start,int count,int after){}public void onTextChanged(CharSequence s,int start,int before,int count){}public void afterTextChanged(Editable value){if(updating[0]||updatingControls||!input.hasFocus())return;try{float parsed=Float.parseFloat(value.toString());if(Float.isFinite(parsed)&&parsed>=min&&parsed<=max){float normalized=min+Math.round((parsed-min)/step)*step;input.setError(null);if(Float.compare(get.get(),normalized)!=0){set.set(normalized);changed();}}}catch(NumberFormatException ignored){}}});
        ...
        slider.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){public void onProgressChanged(SeekBar bar,int value,boolean user){if(user&&!updating[0]){float normalized=min+value*step;if(Float.compare(get.get(),normalized)!=0){set.set(normalized);changed();}}}public void onStartTrackingTouch(SeekBar bar){finishEditing();}public void onStopTrackingTouch(SeekBar bar){flush();}});
    }
    private void colorControl(LinearLayout parent,String title,IntGet get,IntSet set){
        ...
        GradientDrawable swatch=round(get.get(),6);swatch.setBounds(0,0,dp(22),dp(22));reveal.setCompoundDrawables(swatch,null,null,null);reveal.setCompoundDrawablePadding(dp(10));int[] reflected={0};
        reflect(reveal,()->{int color=get.get();if(reflected[0]!=color){reflected[0]=color;swatch.setColor(color);}setText(reveal,title+"  "+WidgetStyle.rgb(color)+(editor.getVisibility()==View.VISIBLE?"  ▴":"  ▾"));});
        Runnable[] opened={null};
        reveal.setOnClickListener(v->{finishEditing();boolean show=editor.getVisibility()!=View.VISIBLE;if(show&&opened[0]==null)opened[0]=buildColorEditor(editor,title,get,set);editor.setVisibility(show?View.VISIBLE:View.GONE);if(show)opened[0].run();requestUpdates(true,false);});
        parent.addView(editor);
    }
    private Runnable buildColorEditor(LinearLayout editor,String title,IntGet get,IntSet set){
        ...
        Runnable reflect=()->{updating[0]=true;int color=get.get();if(wheel.getColor()!=color)wheel.setColor(color);float[] value=wheel.getHsv();int progress=Math.round(value[2]*100);if(brightness.getProgress()!=progress)brightness.setProgress(progress);if(!hex.hasFocus())setText(hex,WidgetStyle.rgb(color));updating[0]=false;};reflect(editor,reflect);
        IntSet picked=color->{if(hex.hasFocus())finishEditing();hex.setError(null);if(get.get()!=color){set.set(color);changed();}else requestUpdates(true,false);};
        ...
        brightness.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){public void onProgressChanged(SeekBar bar,int value,boolean user){if(user&&!updating[0]){wheel.setBrightness(value/100f);picked.set(wheel.getColor());}}public void onStartTrackingTouch(SeekBar bar){finishEditing();}public void onStopTrackingTouch(SeekBar bar){rememberColor(get.get());recentRefresh.run();flush();}});
        Runnable commit=()->{try{int value=WidgetStyle.parseRgb(hex.getText().toString());boolean different=get.get()!=value;set.set(value);hex.setError(null);rememberColor(value);recentRefresh.run();if(different)changed();else requestUpdates(true,false);flush();}catch(IllegalArgumentException e){hex.setError(tr("#RRGGBB 형식으로 입력해 주세요.","Enter a color in #RRGGBB format."));}};
        hex.addTextChangedListener(new TextWatcher(){public void beforeTextChanged(CharSequence s,int start,int count,int after){}public void onTextChanged(CharSequence s,int start,int before,int count){}public void afterTextChanged(Editable value){if(updating[0]||updatingControls||!hex.hasFocus())return;try{int color=WidgetStyle.parseRgb(value.toString());hex.setError(null);if(get.get()!=color){set.set(color);changed();}}catch(IllegalArgumentException ignored){}}});
        hex.setOnFocusChangeListener((v,focused)->{if(!focused)commit.run();});hex.setOnEditorActionListener((view,action,event)->{if(action==EditorInfo.IME_ACTION_DONE){commit.run();hex.clearFocus();return true;}return false;});
    ...
    private void rememberColor(int color){LinkedHashSet<Integer> values=new LinkedHashSet<>();values.add(color);for(int value:recentColors())values.add(value);StringBuilder saved=new StringBuilder();int i=0;for(int value:values){if(i++==6)break;if(saved.length()>0)saved.append(',');saved.append(WidgetStyle.rgb(value));}android.content.SharedPreferences prefs=getSharedPreferences(appearanceUiPrefs(),MODE_PRIVATE);if(!saved.toString().equals(prefs.getString("recent_colors","")))prefs.edit().putString("recent_colors",saved.toString()).apply();}
    ...
    private void chooseHomePreviewSize(){
        ...
        new AlertDialog.Builder(this).setTitle(tr("미리보기 크기","Preview size")).setSingleChoiceItems(labels,selected,(dialog,which)->{selectedHomeSizeKey=homePreviewSizes.get(which).key();previewStage.scrollTo(0,0);requestUpdates(false,true);dialog.dismiss();}).setNegativeButton(tr("닫기","Close"),null).show();
    }
    ...
    private void showEffect(String state){if(!current.feedbackEnabled){Toast.makeText(this,tr("효과 표시를 켜 주세요.","Enable refresh indicators to preview them."),Toast.LENGTH_SHORT).show();return;}feedbackState=state;ui.removeCallbacks(endEffect);requestUpdates(false,true);ui.postDelayed(endEffect,current.feedbackDurationMs);}
    private void changed(){appearanceDirty=true;requestUpdates(true,true);ui.removeCallbacks(publish);ui.postDelayed(publish,220);}
    private void requestUpdates(boolean controls,boolean preview){
        controlsDirty|=controls;previewDirty|=preview;
        if(framePosted||rootLayout==null)return;
        framePosted=true;rootLayout.postOnAnimation(renderFrame);
    }
    private void cancelUpdates(){if(rootLayout!=null)rootLayout.removeCallbacks(renderFrame);framePosted=false;controlsDirty=false;previewDirty=false;}
    private void reflect(View view,Runnable update){controlUpdates.add(new ControlUpdate(view,update));}
    private boolean controlVisible(View view){
        for(View currentView=view;currentView!=null;){
            if(currentView.getVisibility()!=View.VISIBLE)return false;
            if(currentView==settings)return true;
            ViewParent parent=currentView.getParent();currentView=parent instanceof View?(View)parent:null;
        }
        return false;
    }
    private void refreshControls(){updatingControls=true;try{for(ControlUpdate update:controlUpdates)if(controlVisible(update.view))update.reflect.run();}finally{updatingControls=false;}}
    private void flush(){ui.removeCallbacks(publish);saveAppearance();}
    private void saveAppearance(){
        if(current==null||!appearanceDirty)return;
        current.normalize();String signature=WidgetAppearance.signature(current);
        if(floating&&(floatingWidth!=savedFloatingWidth||floatingHeight!=savedFloatingHeight)){
            FloatingPreferences.saveSize(this,floatingWidth,floatingHeight);savedFloatingWidth=floatingWidth;savedFloatingHeight=floatingHeight;
        }
        if(!signature.equals(savedAppearance)){
            if(floating)FloatingPreferences.saveStyle(this,current.copy());else WeeklyWidget.saveStyle(this,current.copy());
            savedAppearance=signature;
        }
    ...
    private void choice(LinearLayout parent,String title,String[] names,IntGet get,IntSet set){
        ...
        for(int i=0;i<names.length;i++){final int index=i;Button option=button(options,names[i],()->{boolean different=get.get()!=index;set.set(index);options.setVisibility(View.GONE);if(different)changed();else requestUpdates(true,false);});reflect(option,()->paintSelected(option,get.get()==index));}
        pick.setOnClickListener(v->{finishEditing();options.setVisibility(options.getVisibility()==View.VISIBLE?View.GONE:View.VISIBLE);requestUpdates(true,false);});
        reflect(pick,()->setText(pick,title+" · "+names[Math.max(0,Math.min(names.length-1,get.get()))]+(options.getVisibility()==View.VISIBLE?" ▴":" ▾")));
    }
    private void toggle(LinearLayout parent,String title,BoolGet get,BoolSet set){Switch control=new Switch(this);control.setText(title);control.setTextColor(TEXT);control.setTextSize(13);control.setPadding(0,dp(12),0,dp(12));control.setMinHeight(dp(48));parent.addView(control,new LinearLayout.LayoutParams(-1,-2));reflect(control,()->control.setChecked(get.get()));control.setOnCheckedChangeListener((view,checked)->{if(!updatingControls){set.set(checked);changed();}});}
    ...
    private LinearLayout accordion(LinearLayout parent,String title,boolean expanded){Button reveal=button(parent,title+(expanded?" ▴":" ▾"),()->{});LinearLayout content=column(parent);content.setVisibility(expanded?View.VISIBLE:View.GONE);reveal.setOnClickListener(v->{finishEditing();boolean show=content.getVisibility()!=View.VISIBLE;content.setVisibility(show?View.VISIBLE:View.GONE);reveal.setText(title+(show?" ▴":" ▾"));requestUpdates(true,false);});return content;}
    ...
    private static void setText(TextView view,String text){if(!text.contentEquals(view.getText()))view.setText(text);}
