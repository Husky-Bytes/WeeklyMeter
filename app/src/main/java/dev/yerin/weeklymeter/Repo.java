...
final class Repo {
    ...
    boolean reconcileConnection()throws Exception{
        Map<String,Object> auth=Json.object(vault.read().get("auth"));
        boolean connected=NetworkPolicy.headerSafe(Json.string(auth.get("access")));
        Store.prefs(c).edit().putBoolean("connected",connected).apply();
        return connected;
    }
    void finishBrowserLogin(String code,String verifier)throws Exception{
        ...
        if(!Json.object(state.get("auth")).isEmpty())throw new IllegalStateException("이미 연결된 계정이 있어.");
        ...
        state.remove("pending");state.remove("login_retry_at");state.put("auth",auth);saveCredentials(state);
        Store.prefs(c).edit().putBoolean("connected",true).putString("error","").remove("meters").remove("attempt").remove("backoff").putString("selected","codex").apply();
    }
    ...
    private Map<String,Object> refresh(Map<String,Object> state,Map<String,Object> auth)throws Exception{
        ...
        if(r.isEmpty())throw new IllegalStateException("다시 로그인이 필요해. 연결을 지우고 로그인해 줘.");
        Map<String,Object> response=Api.postJson("/oauth/token",Json.map("grant_type","refresh_token","client_id",Api.CLIENT_ID,"refresh_token",r));
        // The server may already have invalidated the old refresh token.
        Map<String,Object> updated=normalize(response,auth);state.put("auth",updated);saveCredentials(state);return updated;
    }
